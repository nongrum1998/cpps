import { handleRequestToken } from '@utils/http/request-interceptor';
...
describe('createRequestInterceptor', () => {
  it('attaches the Bearer token when stored and no header is set', async () => {
    ...
    const result = await handleRequestToken()(makeConfig());
  ...
  it('encrypts plain-object bodies with the payload wrapper', async () => {
    ...
    await handleRequestToken()(makeConfig({ data: { ppo_no: '123' } }));
  ...
  it('passes FormData bodies through unchanged', async () => {
    ...
    const result = await handleRequestToken()(makeConfig({ data: formData }));
  ...
  it('passes URLSearchParams bodies through unchanged', async () => {
    ...
    const result = await handleRequestToken()(makeConfig({ data: params }));
